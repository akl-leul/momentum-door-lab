...
// For a sliding mass: L = m * r² * ω (rotational) + m * r * v_r (radial contribution)
// where v_r is the radial velocity of the mass relative to the door
export function calculateMassAngularMomentum(
  mass: number,
  position: number,
  doorAngularVelocity: number,
  massVelocity: number = 0
): number {
  // Rotational component: m * r² * ω
  const rotationalL = mass * position * position * doorAngularVelocity;
  // Radial component: m * r * v_r (cross product is zero since r and v_r are parallel)
  // L = m * (r x v) = m * (r x (v_tan + v_rad)) = m * (r x v_tan) + m * (r x v_rad)
  // r x v_rad = 0 because they are parallel vectors.
  // Therefore, only tangential velocity contributes to angular momentum.
  return rotationalL;
}
...
export function updatePhysics(state: PhysicsState, dt: number): PhysicsState {
  ...
  if (state.useCounterMass) {
    ...
    const minPos = 0.15; // Allow more sliding range
    const maxPos = state.doorWidth - 0.02; // Extend further to end
...
export function createInitialState(
  ...
  const state: PhysicsState = {
    ...
    doorMass,
    doorWidth,
    massPosition: 0.15, // Absolute minimum for extreme braking effect
    massVelocity: 0,
    counterMass,
    frictionCoefficient,
    windTorque,
